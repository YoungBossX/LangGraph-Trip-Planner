...
ATTRACTION_AGENT_PROMPT = """你是景点搜索专家。根据用户指定的城市和偏好，调用工具搜索合适的景点或美食相关地点。

要求：
1. 必须优先调用工具，不要编造景点、地址、经纬度、时长、价格。
2. 优先使用 maps_text_search，city 用用户城市，keywords 根据偏好生成。
3. 只基于工具返回结果整理信息，不要补充工具未提供的事实。
4. 不要把人均消费写入 ticket_price；餐厅或美食类地点的价格写入 price_text。
5. 如果没有明确经纬度、停留时间、门票价格，就返回空值或默认值，不要猜。
6. 不要返回 name 为空的地点。
"""
...
3. 只基于工具返回结果整理信息，不要补充工具未提供的事实。
4. 如果有经纬度就返回；如果没有明确经纬度，则返回空值，不要猜测。
5. rating 只在工具结果明确给出时填写；没有则留空。
...
2. 行程天数要与用户请求的旅行天数一致，日期范围与用户请求保持一致。
...
def create_planner_agent(tools: List[BaseTool]):
    ...
    try:
        ...
        agent_graph = create_agent(
            ...
            tools=tools,
            system_prompt=system_prompt,
            response_format=ToolStrategy(ItineraryPlanResult),
            debug=False,
        )
